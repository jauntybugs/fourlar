      ...
      <section className="py-20 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { icon: <Users className="text-blue-600" />, title: 'Principal Designer Advisory', desc: 'Expert coordination and design risk management aligned with CDM 2015.' },
              { icon: <ClipboardList className="text-blue-600" />, title: 'Pre-Construction Info', desc: 'Streamlined information management to ensure project clarity and safety.' },
              { icon: <Construction className="text-blue-600" />, title: 'Construction Phase Plans', desc: 'Comprehensive, project-specific safety roadmaps for site operations.' },
              { icon: <Search className="text-blue-600" />, title: 'RAMS Review', desc: 'Rigorous vetting of method statements to ensure suitable hazard controls.' },
              { icon: <ShieldCheck className="text-blue-600" />, title: 'Site Safety Inspections', desc: 'Objective on-site audits to identify compliance gaps and behavioral risks.' },
              { icon: <BadgeCheck className="text-blue-600" />, title: 'Retainer-Based Support', desc: 'Ongoing, outsourced safety leadership tailored to your business scale.' }
            ].map((service, i) => (
              <div key={i} className="bg-white p-8 rounded-2xl shadow-sm hover:shadow-xl transition-all border border-slate-100 group">
                <div className="mb-6 p-3 bg-blue-50 w-fit rounded-lg group-hover:bg-blue-600 group-hover:text-white transition-all duration-300">
                  {React.cloneElement(service.icon as React.ReactElement<any>, { size: 24 })}
                </div>
                <h4 className="text-xl font-bold text-slate-900 mb-3">{service.title}</h4>
                <p className="text-base text-slate-600 mb-6 leading-relaxed">{service.desc}</p>
                <Link to="/services" className="text-blue-600 font-bold flex items-center gap-2 text-sm uppercase tracking-widest hover:underline decoration-2 underline-offset-4">
